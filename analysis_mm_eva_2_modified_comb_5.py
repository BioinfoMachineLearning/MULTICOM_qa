...
multi_eva_dir =  "/home/bdmlab/blank/score/comb_5_csv/"
output_dir= "/home/bdmlab/"
list_of_file = file_reader("/home/bdmlab/casp_14.list")
for _targets in list_of_file:
    ...
    for row in data_eva.iterrows():
        ...
        # temp.comb4_Score = row[1].comb4_Score
        # temp.comb4_RANK = row[1].comb4_RANK
        temp.contact_rank = row[1].contact_Rank
        temp.comb5_final_rank_1 = row[1].Final_Rank
        temp.comb5_final_rank_2 = row[1].Final_Rank_2
    ...
    # comb_4_score_top_1_name = sorted(eva_score_list.items(), key=lambda x: x[1].comb4_Score, reverse=True)[0]
    # comb_4_score_top_1_score = comb_4_score_top_1_name[1].true_mm_score
    # loss_comb_4_score_top_1_score = float(true_top_1_score) - float(comb_4_score_top_1_score)
    #
    # comb_4_rank_top_1_name = sorted(eva_score_list.items(), key=lambda x: x[1].comb4_RANK)[0]
    # comb_4_rank_top_1_score = comb_4_rank_top_1_name[1].true_mm_score
    # loss_comb_4_rank_top_1_score = float(true_top_1_score) - float(comb_4_rank_top_1_score)
# '''
#         temp.contact_rank = row[1].contact_Rank
#         temp.comb5_final_rank_1 = row[1].Final_Rank
#         temp.comb5_final_rank_2 = row[1].Final_Rank_2
# '''
    contact_Rank_top_1_name = sorted(eva_score_list.items(), key=lambda x: x[1].contact_rank)[0]
    cs_top_1_score = contact_Rank_top_1_name[1].true_mm_score
    cs_top_1_loss_score = float(true_top_1_score)-float(cs_top_1_score)


    Final_Rank_top_1_name = sorted(eva_score_list.items(), key=lambda x: x[1].comb5_final_rank_1)[0]
    final_top_1_score = Final_Rank_top_1_name[1].true_mm_score
    Final_Rank_top_1_loss_score = float(true_top_1_score)-float(final_top_1_score)

    Final_Rank_2_top_1_name = sorted(eva_score_list.items(), key=lambda x: x[1].comb5_final_rank_2)[0]
    Final_Rank_2_top_1_score = Final_Rank_2_top_1_name[1].true_mm_score
    Final_Rank_2_top_1_loss_score = float(true_top_1_score)-float(Final_Rank_2_top_1_score)
    ...
    final_results.append(
        [Title.replace(".csv",""),  icps_1_score, recall_top_1_score, mm_align_top_1_score,cs_top_1_score, final_top_1_score,Final_Rank_2_top_1_score, true_top_1_score,loss_icps_top_1_score,loss_recall_top_1_score,loss_mm_align_top_1_score,cs_top_1_loss_score,Final_Rank_top_1_loss_score,Final_Rank_2_top_1_loss_score])

# print(final_results)

name_of_output_file = output_dir + str("casp14_comb5") + ".csv"
_header_row = ['Target',"icps_1_score","recall_top_1_score","mm_align_top_1_score",'cs_top_1_score','final_top_1_score','final_top_2_score',"true_top_1_score",'icps_loss','recall_loss','mm_align_loss','contact_score_loss','rank_1_score','rank_2_rank']
