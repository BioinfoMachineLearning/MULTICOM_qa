...
output_dir = "/home/bdmlab/MM_Eva/casp14/H1036/output_new_2/"
CPU_COUNT=10
...
for pdb_values in pdb_profile_dict:
    ...
    total_contacts = 0
    total_len = 0
    # len(temp_pdb_profile.chain_fasta)
    for _fastas in temp_pdb_profile.chain_fasta:
        # print(_fastas)
        total_len = total_len + len(temp_pdb_profile.chain_fasta.get(_fastas))
    for dimer in valid_dimer_combos:
        ## get all the dimer cmaps
        ##for each calculate check if empty and also take the maximum
        dimer_cmap_dir = predicted_structures_dimer_cmap_dir + "sequence_" + str(dimer) + "/"
        all_cmap_same_type = eva_util.specific_filename_reader(dimer_cmap_dir, pdb_values)
        for _cmaps in all_cmap_same_type:
            dimer_cmap_file_name = dimer_cmap_dir + str(_cmaps) + ".cmap"
            if os.path.exists(dimer_cmap_file_name):
                total_contacts= total_contacts+np.sum(np.loadtxt(dimer_cmap_file_name))
    x = total_contacts/total_len
    # print(x)
    temp_sig =  1/(1 + np.exp(-x))
    # print(str("sig_")+str(temp_sig))
    pdb_profile_dict.get(pdb_values).contact_scores = temp_sig
print("Generating Final scoring part")
eva_util.print_final_data_new_comb5(_file_name=output_dir + "/" + str(os.path.basename(monomer_sequences_dir).split(".")[0]) + ".csv",
