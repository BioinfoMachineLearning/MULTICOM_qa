...
class eva_score:
    ...
    pass
multi_eva_dir = "/home/bdmlab/MM_Eva/casp14/casp14_Multi_eva_results/"
output_dir= "/home/bdmlab/MM_Eva/casp14/Multi_eva_results_modified_comb_4/"
...
for pro_values in list_of_file:
    ...
    rank_recall_1_name = sorted(eva_score_list.items(), key=lambda x: x[1].recall, reverse=True)
    counter =1
    ...
    counter =1
    for values in rank_recall_1_name:
        eva_score_list[values[1].name].recall_rank = counter
        # print(values[1].mm_align_score)
        counter = counter +1
        # ms_top_1_score = ms_top_1_name[1].tr
    ...
    for values in eva_score_list:
        ...
        temp.new_final_score = np.average([temp.icps_score   , temp.mm_align_score,temp.recall])
        temp.new_final_rank = np.average([temp.icps_rank,  temp.mm_rank ,temp.recall_rank])
        # print(values[1])
        # print("here")
    _header_row = ["Name",	"average_MS",	"average_DS",	"average_ICPS",	"average_R",	"average_MMS",	"Final_score", "comb4_Score",	"ICPS_Rank","MM_RANK","recall_RANK","comb4_RANK"]
    with open(output_dir+"/"+pro_values+".csv", 'w') as csvfile:
        ...
        for data in eva_score_list:
            ...
            filewriter.writerow([data,temp.monomer_score,temp.ds_score,temp.icps_score,temp.recall,temp.mm_align_score,temp.final_score,temp.new_final_score,temp.icps_rank,temp.mm_rank,temp.recall_rank,temp.new_final_rank])
